...
export default class TaskDetails {
    ...
    async init() {
        ...
        if (this.token !== null) {
            ...
            // Get creator
            const creatorObj = await this.services.getUser(this.token, this.task.creator_user_id);
            this.creator = creatorObj.first + " " + creatorObj.last;

            // Get assignee
            if (this.task.assigned_to !== null) {
                const assigneeObj = await this.services.getUser(this.token, this.task.assigned_to);
